...
export class ListAgentsController {
  ...
  buildAgentsList() {
    ...
    for (let a in this.availableAgents) {
        let agentName = this.availableAgents[a];
        let isEnabled = this.isEnabled(agentName, this.agents);
        this.agentsList.push({"name" : agentName, "isEnabled" : isEnabled});
    }
  }
