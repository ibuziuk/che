...
public abstract class ServerEvaluationStrategy {
    ...
    /**
     * @return true <strong>external</strong> addresses need to be exposed against https, false otherwise
     */
    protected abstract boolean useHttpsForExternalUrl();
     ...
     * @return https, if {@link #useHttpsForExternalUrl()} method in sub-class returns true and protocol for internal Url is http
     */
    private String getProtocolForExternalUrl(final String protocolForInternalUrl) {
        if (useHttpsForExternalUrl() && HTTP.equals(protocolForInternalUrl)) {
            return HTTPS;
        }
